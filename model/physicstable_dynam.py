...
import pygame as pg
import random, math
from padlib import DashedLine # http://www.geometrian.com/Projects.php
...
class PhysicsTable(object):
    ...
    def __init__(self, dims, background_cl = WHITE, wall_cl = BLACK, ball_cl = BLUE, ball_rad = 20, openend = None, badzone = None, active = True, jagged = True):
        self.dim = dims
        self.wall_c = wall_cl
        ...
        # Variable to hold the location of the paddle when the ball hits the target
        self.padend = None
        
        # Should the edges be drawn as jagged lines?
        self.jag = jagged

        # Add an outer wall & catch area (ADD CATCH AREA & LOGIC FOR UNBOUNDING)
        self.addWall((-50,-100),(self.dim[0]+50,2),TOP) # top
        self.addWall((-100,0),(2,self.dim[1]),LEFT) # left
        self.addWall((-50,self.dim[1]-2),(self.dim[0]+50,self.dim[1]+100),BOTTOM) # bottom
        self.addWall((self.dim[0]-2,0),(self.dim[0]+100,self.dim[1]),RIGHT) # right
    ...
    def addWall(self,upleft,botright,dir = None, elastic = True):
        w = botright[0] - upleft[0]
        h = botright[1] - upleft[1]
        self.walls.append(self.Wall(pg.Rect(upleft,(w,h)),dir,self.wall_c,elastic))

    def addPaddle(self,initpos,length,h_or_v,fixeddir = True):
        self.paddle = self.Paddle(initpos,length,h_or_v,fixeddir,self.wall_c,self.dim)
    ...
    def draw(self):
        ...
        for w in self.walls: w.draw(self.surface,self.jag, self.bk_c)
        if self.ball: self.ball.draw(self.surface)
        if self.paddle: self.paddle.draw(self.surface, self.act)
        if self.occlude: pg.draw.rect(self.surface, pg.Color('Grey'),self.occlude)

        return self.surface

    def step(self,t = 1/50.0, control = True, mouse = None, stp = 1, xtarg = None):
        if mouse:
            self.paddle.move(mouse)
        # Note: should correct this for some overflow fractions at some point...
        for i in range(int(t*1000/stp)):
            r = self.physicsstep(stp, xtarg)
    ...
    def catches(self, good_col = GREEN, bad_col = RED):
        if self.ball:
            r = self.checkcatch(self.ball,self.bz)
            if r == True:
                self.ball.stop()
                self.ball.col = good_col
            elif r == False:
                self.ball.stop()
                self.ball.col = bad_col
        else: r = None
        return r

    def fails(self,failrect):
        if self.ball and self.act:
            b = self.ball
    ...
    def physicsstep(self,s = 1, xtarg = None):
        self.sincecol += s
        if self.ball:
            b = self.ball
            # Move the ball
                ...
                if frcl:
                    ...
                    if c != False:
                        ...
                        # Elastic collision: 
                            ...
                            b.bounce(c)
            if self.bz and self.fails(self.bz): return False
            # Test whether the ball has hit the target
            if xtarg and self.paddle:
                if abs(b.getpos()[0]-xtarg) <= 1: self.padend = self.paddle.getcenter()[1]
                
        ret = None
        if self.paddle: ret = self.catches()
        return ret
    ...
    class Wall(object):
        def __init__(self,rect,direction = None, color = BLACK, elastic = True):
            self.r = rect
            self.col = color
            self.e = elastic
            self.dir = direction

        def draw(self,surface, jagged = False, bgcol = WHITE):
            pg.draw.rect(surface,self.col,self.r)
            if jagged:
                d = 0
                if self.dir == LEFT: p1 = self.r.topright; p2 = self.r.bottomright; p1 = (p1[0]+d,p1[1]); p2 = (p2[0]+d,p2[1])
                elif self.dir == RIGHT: p1 = self.r.topleft; p2 = self.r.bottomleft; p1 = (p1[0]-d,p1[1]); p2 = (p2[0]-d,p2[1])
                elif self.dir == TOP: p1 = self.r.bottomleft; p2 = self.r.bottomright; p1 = (p1[0],p1[1]+d); p2 = (p2[0],p2[1]+d)
                elif self.dir == BOTTOM: p1 = self.r.topleft; p2 = self.r.topright; p1 = (p1[0],p1[1]-d); p2 = (p2[0],p2[1]-d)
                else: return None
                DashedLine(surface,self.col,bgcol,p1,p2,3)
    ...
    class Paddle(object):
        def __init__(self,initpos, length, direct = 'h', fixeddir = True, color = BLACK, parentdim = None):
            self.p = initpos
        ...
        def draw(self,surface,active):
            lr = self.getlr()
            if not active: c = pg.Color('Grey')
            else: c = self.col
            pg.draw.line(surface,c,self.p,lr,3)
        ...
        def move(self,mousepos):
            ...
            # Can't go beyond the edge of the screen
            if self.pdim:
                if newx <0: newx = 0
                if newx + self.l[0] > self.pdim[0]: newx = self.pdim[0] - self.l[0]
                if newy <0: newy = 0
                if newy + self.l[1] > self.pdim[1]: newy = self.pdim[1] - self.l[1]
